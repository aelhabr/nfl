...
filepath_import <- str_c("data/", filename, "_cleaned.csv")
...
calculate_pct <- function(x, na_rm = TRUE, digits_round = 2) {
  round((100 * sum(x, na.rm = na_rm) / sum(!is.na(x))), digits_round)
}
...
bet_metrics_check
bet_metrics_check %>% filter(counts_diff_2 != 0)
...
bet_metrics %<>%
  mutate(
    line_b50 = ifelse(line_home <= bet_q_overall$q_50, 1, 0),
    # line_a50 = ifelse(line_home > bet_q_overall$q_50, 1, 0),
    # line_b10 = ifelse(line_home <= bet_q_overall$q_10, 1, 0),
    # line_a90 = ifelse(line_home >= bet_q_overall$q_90, 1, 0),
    line_b25 = ifelse(line_home <= bet_q_overall$q_25, 1, 0),
    line_a75 = ifelse(line_home >= bet_q_overall$q_75, 1, 0)
  )

# For some reason, this is no longer a tibble.
bet_metrics %<>% as_tibble()
...
bet_metrics %>%
  ...
  summarise(
    g = n(),
    line_a50_sum = sum(line_a50, na.rm = TRUE),
    line_b50_count = sum(line_b50, na.rm = TRUE)
  ) %>%
  ungroup()
...
calculate_pct_2 <-
  function(x_1,
           ...
           digits_round = 1) {
    round(100 * (sum((
      x_1 == value_1 & x_2 == value_2
    ), na.rm = na_rm) /
      sum(!is.na(x_1) & x_2 == value_2
      )),
    digits_round)
  }
...
bet_metrics_summary_2 <-
  ...
  summarise(
    g = n(),
    g_a50_count = sum(line_b50 == 0),
    g_b50_count = sum(line_b50 == 1),
    hfw_a50_count = sum(hfw == 1 & line_b50 == 0, na.rm = TRUE),
    hfw_b50_count = sum(hfw == 1 & line_b50 == 1, na.rm = TRUE),
    hfw_b25_count = sum(hfw == 0 & line_b25 == 1, na.rm = TRUE),
    huw_a75_count = sum(huw == 1 & line_a75 == 1, na.rm = TRUE),
    hfw_a50_pct = calculate_pct_2(hfw, line_b50, 1, 0),
    hfw_b50_pct = calculate_pct_2(hfw, line_b50, 1, 1),
    hfw_b25_pct = calculate_pct_2(hfw, line_b25, 1, 1),
    huw_a75_pct = calculate_pct_2(huw, line_a75, 1, 1)
  )
bet_metrics_summary_2
tail(bet_metrics_summary_2)
glimpse(bet_metrics_summary_2)

cols_tidy_idx_2 <-
  names(bet_metrics_summary_2) %>%
  str_which("_[a-b][0-9]5_pct$")
cols_tidy_idx_2
...
bet_metrics_summary_2 %>%
  ...
  ggplot(aes(x = season)) +
  geom_line(aes(y = value, group = key, color = key)) +
  geom_smooth(aes(y = value, group = key, color = key), method = lm, se = FALSE, size = 2) +
  labs(
    title = "Win % ATS For Heaviest Favorites/Underdogs",
    y = "",
    x = ""
  ) +
  theme_minimal() +
...
check_names(bet_metrics_filtered, game_picks_2)
# check_names(bet_metrics_filtered, game_picks_2, -1)
...
calculate_pct_3 <-
  function(x_1,
           ...
           digits_round = 1) {
    round(100 * (sum((
      x_1 == value_1 & !is.na(x_2)
    ), na.rm = na_rm) /
      sum(!is.na(x_1) & !is.na(x_2)
      )),
    digits_round)
  }
...
game_picks_calc_season_2 <-
  game_picks_join_tidy %>%
  group_by(season, person, bet_type) %>%
  summarise(
    pick_correct_count_spread = sum(pick_result_spread == 1 & !is.na(value), na.rm = TRUE),
    pick_correct_pct_spread = calculate_pct_3(pick_result_spread, value)
  ) %>%
  ungroup()
game_picks_calc_season_2

game_picks_calc_season_2 %>%
  filter(bet_type %in% Hmisc::Cs(huw, hfw)) %>%
  ggplot() +
  geom_col(
    aes(
      x = season, y = pick_correct_pct_spread,
      group = person, fill = person),
    position = "dodge"
  ) +
  facet_wrap( ~ bet_type) +
  labs(
    title = "Pick Correct % By Bet Type",
    y = "",
    x = ""
  ) +
  theme_minimal()
